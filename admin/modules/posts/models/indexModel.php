...
function get_post($start, $num_per_page, $where = "")
{
...
{
    $list_post = db_fetch_array("SELECT * FROM `tbl_posts` where post_title like '%{$str}%'");
    return $list_post;
}
function count_post_search($str){
    return db_fetch_row("SELECT COUNT(post_id) as sl FROM `tbl_posts` where post_title like '%{$str}%'");
}
function count_post(){
    return db_fetch_row("SELECT COUNT(post_id) as count_id FROM `tbl_posts`");
}
